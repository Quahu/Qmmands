...
namespace Qmmands
{
    ...
    public sealed class ContainsAttribute : ParameterCheckAttribute
    {
        ...
        {
            if (!(argument as string).Contains(Value, StringComparison))
                return Failure($"The provided argument must contain the {(StringComparison.IsCaseSensitive() ? "case-sensitive" : "case-insensitive")} value: {Value}.");
